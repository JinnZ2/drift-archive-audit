...
Authorship recorded in git: JinnZ2 50, Claude 13.
Regime boundary: commit 47 (`ba6f3d1`, 2026-06-02) — first Claude-authored commit,
after a 204-day gap.
...
| BG-2 | `abb4b49` | 2025-07-10 | Technical-equations.md vs docs/FullHex.md, docs/Blueprint/Ultra-compressed.md | prose names `φ = 1.618` "the BioGrid base tuning constant" (ONE constant, ONE name) while both hex blobs carry `phi1.0008` and the engine uses `this.phi = 1.0008` | **D5 no verbal anchor** | RENDERING collapsed two couplings into one noun; TARGET (two distinct couplings) survived in the hex + the engine | UNATTRIBUTABLE (both encodings are operator-authored at t=0) |
| BG-3 | `abb4b49` | 2025-07-10 | core_brief.md (base64) vs FullHex.md + Ultra-compressed.md (hex) | self-healing recovery `2minutes` vs `20min` | OTHER | RENDERING conflict | UNATTRIBUTABLE |
| BG-4 | gap | 2025-11-10→2026-06-02 | — | 204 days, no commits. Regime changes across the gap: operator-only web-UI commits (`Create X.md`) → Claude-authored branch/PR commits (`feat:`, `fix:`) | OTHER (tooling) | neither | **TOOL_CANDIDATE** — the encoding layer demonstrably changed here; what it did to content is not established |
