...
Authorship recorded in git: Claude 19, JinnZ2 7, `Jinn2Z` 1 (author-name typo, `0aed497`).
Regime boundary: commit 6 (`be471a3`, 2026-03-22, "Add CLAUDE.md") after a 199-day gap.
...
| KC-7 | across corpus | 2025-08 → 2026-09 | data/** | longevity_years / era-span ratio: **mean 0.62 → 0.95**, max 0.88 → 1.00, count over 1.0: **0 → 0** across 42 entries. `hxaro` sits at exactly 1.00 (125 / 125) | **D3 status field dropped** | **RENDERING changed, TARGET survived.** The stated rule (never claim more than the span) holds with zero violations. The unstated one (discount below the span by an epistemic margin) did not: the margin fell 38% → 5%. The uncertainty was carried in a field with no uncertainty type, so it had nothing to hold it | UNATTRIBUTABLE |
